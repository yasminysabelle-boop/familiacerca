...
Deno.serve(async (req: Request) => {
  ...
  async function logRun(opts: { attempted?: number; sent?: number; failed?: number; failureReasons?: Record<string, number>; fatalError?: string }) {
    try {
      await supabase.rpc('log_notification_run', {
        p_function_name: 'send-proof-reminders',
      ...
      console.error('[send-proof-reminders] Failed to log notification run (non-fatal):', logErr)
    }
  }
  ...
  if (logsError) {
    ...
    await logRun({ fatalError: `Error querying logs: ${logsError.message}` })
  ...
  for (const log of logs) {
    ...
    for (const sub of subs ?? []) {
      try {
        ...
        console.error(`[send-proof-reminders] ✗ Push failed for sub ${sub.id}, status=${statusCode}`)
  ...
  console.log(`[send-proof-reminders] Done. sent=${sentCount}, failed=${failCount}`)
