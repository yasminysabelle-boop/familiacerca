...
export default function MedicationHistorialTab({ medications = [], logsByMedId = {}, loading = false }) {
  ...
  return (
    <div style={{ padding: '12px 0 96px', display: 'flex', flexDirection: 'column', gap: 8 }}>
      {days.map(dateKey => {
        ...
        // For each scheduled med, determine its status for this day
        ...
        const hasIssues = confirmedCount < total
            ...
            {isExpanded && (
              <div style={{ padding: '8px 14px 12px', display: 'flex', flexDirection: 'column', gap: 6 }}>
                {medStatuses.map(({ med, status, log }) => {
                  ...
                  const byName      = log?.confirmed_by_name?.split(' ')[0]
                  const givenLate   = log?.given_on_time === false
                  const minsLate    = log?.minutes_late

                  return (
                    <div key={med.id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 10px', borderRadius: 10, background: isConfirmed ? '#F0FDF4' : isOmitted ? '#FFFBEB' : '#F9FAFB' }}>
                      <span style={{ fontSize: 16, flexShrink: 0 }}>
                        {isConfirmed && !givenLate ? '✅' : isConfirmed && givenLate ? '⚠️' : isOmitted ? '❌' : '🔵'}
                      </span>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <p style={{ fontSize: 13, fontWeight: 600, color: '#1A1A1A', margin: 0 }}>{med.name}</p>
                        {confTime && (
                          <p style={{ fontSize: 11, color: '#6B7280', margin: '1px 0 0' }}>
                            🕐 {confTime}{byName ? ` · por ${byName}` : ''}
                          </p>
                        )}
                      </div>
                      <span style={{
                        fontSize: 11, fontWeight: 700, flexShrink: 0,
                        color: isConfirmed && !givenLate ? '#15803D'
                          : isConfirmed && givenLate ? '#D97706'
                          : isOmitted ? '#92400E' : '#6B7280',
                      }}>
                        {isConfirmed && !givenLate ? 'A tiempo'
                          : isConfirmed && givenLate ? `Tarde ${minsLate}min`
                          : isOmitted ? 'Omitido'
                          : 'Sin registro'}
                      </span>
                    </div>
